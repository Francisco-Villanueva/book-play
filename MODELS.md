...
**Notes**:
- A booking generated by a turno fijo is an ordinary booking; `recurringBookingId` is
  the only difference (BR-028)
...
Represents a **turno fijo**: the same court, weekday and start time, week after week.

**Purpose**: Let a venue keep its regular customers without re-entering the booking
every week. It is a *rule that generates bookings*, not a booking itself — it never
blocks availability on its own (BR-028).
...
```typescript
{
  ...
  businessId: UUID (FK -> Business)
  courtId: UUID (FK -> Court)
  userId?: UUID (FK -> User, null for the usual case: a client with no account)

  guestName?: string
  guestPhone?: string
  guestEmail?: string

  dayOfWeek: integer          // 0 = Sunday, same as Date.getDay()
  startTime: time             // endTime is derived from court.slotDuration (BR-004)
  startDate: date
  endDate?: date              // null = no end; the cron keeps extending it

  generatedUntil: date        // how far instances are materialised — makes the cron idempotent
  status: 'ACTIVE' | 'ENDED'

  notes?: string
  createdBy?: UUID            // staff member who created it
  cancellationTokenHash?: string  // SHA-256; the plain token only travels in the email
...
**Constraints**:
- One weekday per series. "Tuesdays and Thursdays" is two series.
- `dayOfWeek` is derived from `startDate`, never sent separately — two fields could
  contradict each other.

**Relationships**:
- Belongs to one `Business`, one `Court`, optionally one `User`
- Has many `Booking` (its instances, via `Booking.recurringBookingId`)

**States**:
- `ACTIVE`: the cron keeps the 12-week window filled
- `ENDED`: future instances were cancelled; past ones stay in history
...
> `RecurringBooking` salió de esta lista: está implementado (BR-028). Ver la sección
> **RecurringBooking** en Core Entities.
